...
Ainda acerca do espaço de amostragem, um detalhe geométrico é decisivo neste problema. O espaço das carteiras não é um cubo livre. Como os pesos precisam somar a unidade e são não negativos, o espaço válido é restrito. Com dois ativos, os pesos vivem sobre um segmento de reta; com três, sobre um triângulo; em geral, sobre a figura chamada simplex. Formalmente,
\begin{equation}
\Delta^{n-1} = \Big\{ \mathbf{w} \in \mathbb{R}^n : w_i \geq 0, \ \textstyle\sum_{i=1}^{n} w_i = 1 \Big\}.
\end{equation}
No caso de três ativos, cada peso varia de $0$ a $1$, e a exigência de que os três somem exatamente $1$ elimina um grau de liberdade, recortando do cubo de possibilidades uma face triangular. Fixados dois pesos, o terceiro fica determinado por diferença, o que explica a redução de dimensão e o formato triangular. A consequência prática é que gerar pontos de Sobol ou LHS num cubo e depois apenas normalizá-los para que somem $1$ distorce o espalhamento, concentrando pontos artificialmente no centro do simplex e destruindo as boas propriedades de cobertura dessas técnicas. Considera-se então que o procedimento mais adequado é gerar os pontos já respeitando a geometria do simplex.
