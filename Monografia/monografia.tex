...
Como a carteira ótima de média-variância \cite{markowitz1952portfolio} é muito sensível a $\boldsymbol{\mu}$, carteiras calculadas com médias históricas cruas costumam ser instáveis, concentradas em poucos ativos e decepcionantes fora da amostra usada para estimar a média de retorno. Essa fragilidade é discutida ao longo do projeto e uma forma de tratá-la é abordada na seção \ref{sec: CAPM}.
...
\subsection{Covariância e Diversificação}
...
A Equação~\eqref{eq:varport} justifica matematicamente a diversificação e é a razão pela qual toda a teoria de portfólios de \citeonline{markowitz1952portfolio} foi elaborada. O risco da carteira, na formulação de Markowitz, não é a média dos riscos individuais: ele depende crucialmente dos termos cruzados $w_i w_j \sigma_{ij}$ referentes a duas diferentes ações e a a forma como variam uma em relação à outra. Quando dois ativos têm covariância baixa ou negativa, suas oscilações se cancelam parcialmente, e a variância do conjunto fica abaixo da média ponderada das variâncias individuais. É possível, assim, montar uma carteira menos arriscada que qualquer um de seus ativos isolados, sem sacrificar o retorno médio. 

A consequência direta dessa descoberta é a conclusão lógica de que, ao analisar o retorno e risco no nível de portfólio, é possível, definindo uma medida de risco adequada, encontrar uma configuração de carteira que minimize o risco assumido para um dado nível de retorno aproveitando-se da interação entre os diversos ativos. Assim, as seções a seguir discutirão justamente estudos acerca de alguns dos principais métodos para a quantificação de risco e retorno e de como realizar a otimização de um portfólio.

\subsection{Medidas de Risco}

Entende-se do exposto até o momento que o cerne da questão de como diversificar eficientemente um portfólio é a identificação do risco das diferentes combinações de pesos e a sua minimização.
Para conseguir minimizar esse risco é necessário então definí-lo como uma função $\mathrm{Risco}(\mathbf{w})$ a ser otimizada. 
\citeonline{markowitz1952portfolio}, foi quem notou que isso poderia ser feito por meio da minimização da covariância ponderada pelos pesos dos ativos, no entanto, estudos posteriores apontaram alternativas para lidar com problemas encontrados em sua formulação. 

Nesta seção serão abordadas as duas medidas de risco e os problemas de otimização associados a elas; 
primeiro a elaborada por Markowitz em 1952 (média-variância), por sua relevância ao campo; e em seguida o \textit{Conditional Value-at-Risk} (CVaR), em português, Valor-em-Risco Condicional, por ser a métrica principal a ser utilizada no projeto.

\subsubsection{Média-Variância}
\label{sec: med var}

Reunindo os elementos tratados anteriormente, 
a seleção de portfólio proposta por Markowitz (1952) 
pode ser escrita como um problema de otimização do risco representado pela variância da carteira. 
Veja a seguir uma formulação comum que minimiza o risco sujeito a um retorno alvo $\mu^\ast$:
\begin{equation}
...
\label{eq:markowitz}
\end{equation}
Entende-se da formulação de Markowitz (média-variância), que para evitar altos riscos na carteira devemos reduzir a volatilidade conjunta dela, 
e que isso pode ser feito minimizando a covariância ponderada pelos pesos cruzados entre os ativos nela. 

Considera-se também que essa minimização esta sujeita a um objetivo de retorno, isso porque a minimização exclusiva do risco não nos garante nada sobre o seu retorno associado. Assim, o problema de otimização envolve também essa restrição responsável por delimitar o campo de riscos encontrados a um mesmo resultado de retorno. Por isso, entende-se que variando esse alvo $\mu^\ast$, 
o conjunto de soluções encontradas descreve uma fronteira contendo a carteira menos arriscada para a restrição desejada, sendo assim a carteira ideal para o nível de retorno escolhido. Varrendo todos os possíveis retornos esperados gera-se uma curva com os menores riscos para cada um deles. Essa curva é chamada de fronteira eficiente do plano de risco-retorno definido. Em outras palavras, cada ponto da fronteira é a carteira de menor variância para aquele nível de retorno. A escolha do portfólio final depende da tolerância ao risco do investidor.

Compreendido o funcionamento da abordagem proposto por \citeonline{markowitz1952portfolio} é importante ressaltar alguns problemas presentes nela. A formulação possui 3 principais pontos negativos: 
\begin{itemize}
  \item \textbf{Simetria Indesejada:} A variância penaliza desvios indiscriminadamente, considerando movimentos positivos e negativos causados por volatilidade como sendo igualmente ruins. 
  Esta penalização não faz sentido para o investidor. Uma carteira com muitos retornos excepcionalmente altos pode ser rotulada como "arriscada" pela variância mesmo que ela nunca produza perdas relevantes.
  \item \textbf{Suposição de Normalidade:} O método considera que é possível descrever o comportamento de retornos apenas com média e variância, o que é válido apenas quando a distribuição de retornos é uma normal. 
  Distribuições de retornos em ativos reais podem possuir caudas pesadas e desviar significativamente da normal, concentrando nos extremos justamente o risco que média e variância deixam de capturar.
  \item \textbf{Sensibilidade aos Parâmetros Estimados:} A otimização recebe como entrada os retornos esperados e a matriz de covariância, que são estimativas sujeitas a erro, mas os trata como valores exatos. 
  Isso tende a concentrar peso nos ativos cujos retornos foram superestimados por acaso, produzindo carteiras instáveis e pouco diversificadas que se alteram de forma brusca diante de pequenas variações nos dados. Uma mudança de amostra inicial pode reconfigura significatvamente a carteira selecionada.
\end{itemize}
...
Os retornos de ativos financeiros costumam apresentar dois comportamentos que fogem 
da distribuição normal, e por isso estão desalinhados com as suposições da formulação de média-variância, como citado anteriormente. 
O primeiro é a presença de caudas pesadas: quedas e altas muito grandes acontecem com uma frequência 
maior do que uma distribuição normal preveria. Na prática, isso significa que eventos que a normal 
trataria como praticamente impossíveis, como uma queda de vários desvios padrão em um único dia, 
ocorrem no mercado real com uma frequência bem maior do que essa suposição sugere. O segundo é a 
assimetria: os movimentos de queda podem ser mais abruptos e mais frequentes do que movimentos de 
alta de magnitude equivalente. A consequência prática dessas duas características é que se as 
desconsiderarmos iremos penalizar de forma incoerente os riscos observados por meio da série temporal 
do preço da carteira de ações \cite{fabozzi2008portfolio, rockafellar2000optimization}. 

Além dessa questão da suposição de normalidade, existe também, como comentado na subseção anterior, a questão da indiscriminação com relação à pensalização ao passo que a variância registra da mesma forma um desvio para cima e um desvio para baixo, quando na realidade o investidor só enxerga como arriscado o desvio para baixo. 
Duas carteiras podem ter a mesma variância e retorno esperado e, ainda assim, exposições muito diferentes a perdas grandes, se uma delas tiver retornos com cauda mais pesada do que a outra. 
Tendo em vista esse problema, novas funções de risco foram definidas para evitar essa premissa de simetria indesejável, como por exemplo o VaR
\footnote{O VaR, precursor e base fundamental para a criação do CVaR, quantifica a perda máxima a um determinado nível de confiança $\alpha$. Foi substituído pelo CVaR por ignorar a magnitude das perdas associadas ao peso da cauda (risco de cauda) e por não ter a propriedade de subaditividade \cite{barrosa2015aplicacao}.} 
e o CVaR. Ambas as funções estão exemplificadas de forma ilustrativa na Figura \ref{fig:cvar_var}, onde VaR e CVaR são identificados justamente em um caso onde as premissas de normalidade (curva pontilhada) e simetria, causariam problemas para outras funções objetivo. 
...
O Valor-em-Risco Condicional, ou CVaR, é uma medida de risco pensada para focar exatamente no ponto cego da variância. Em vez de resumir toda a distribuição em desvios padrão, o CVaR olha apenas para a fatia mais extrema das perdas e calcula a perda média dentro dessa fatia \cite{rockafellar2000optimization}. 
O tamanho dessa fatia é definido por um nível de confiança $\alpha$: por exemplo, com $\alpha$ igual a $95\%$, olha-se apenas para os $5\%$ piores cenários de perda e calcula-se a perda média entre eles. Formalmente, sendo $L$ a perda da carteira e $\mathrm{VaR}_\alpha$ o quantil de nível $\alpha$ da distribuição de perdas, que separa os piores cenários do restante,
\begin{equation}
\mathrm{CVaR}_\alpha = \mathbb{E}\big[\, L \mid L \geq \mathrm{VaR}_\alpha \,\big].
\end{equation}
Quanto maior o $\alpha$ escolhido, mais a medida se concentra nos cenários mais raros e mais catastróficos, o que a torna mais sensível justamente às caudas pesadas ignoradas na formulação anterior \cite{rockafellar2000optimization}.
...
Finalmente, com o objetivo de resumir o CVaR de forma comparativa, concluindo esta subseção, a tabela 1 mostra uma visão sucinta de suas características. A leitura da tabela unida à figura \ref{fig:cvar_var} ilustra como a abordagem assimétrica da função objetivo CVaR permite uma consideração mais fiel do problema de cauda em portfólios de investimento.

\begin{table}[H]
\centering
\caption{Comparação entre média-variância e CVaR como medidas de risco}
...
\midrule
Média-variância & Simétrica & Variância da carteira & Ignora a cauda. Usa a variância como risco. \\
\addlinespace
CVaR & Assimétrica & Perda potencial & Foca na cauda. Calcula a perda média nos piores cenários. Sensível a caudas pesadas. \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Incluindo o Retorno na Otimização}

Até aqui o retorno apareceu apenas como um alvo a ser respeitado enquanto se minimizava o risco. Esta seção trata de como o retorno entra de fato na otimização. Começa-se pela forma mais simples, em que ele figura como uma restrição externa ao problema, e avança-se para as formulações que combinam risco e retorno em um único número, as chamadas \textit{Ratios} (Razões), discutindo primeiro o índice de Sharpe, construído sobre a variância, e depois o índice STARR, que adapta essa ideia à medida de risco adotada no projeto, o CVaR. Todas essas formulações, no entanto, compartilham uma mesma dependência: elas tomam o retorno esperado $\boldsymbol{\mu}$ como um dado já disponível. Por isso, a subsection se encerra retomando a dificuldade de estimar esse retorno, já apontada na Subseção~\ref{sec: retorno esperado}, e apresentando o CAPM como uma resposta possível a ela.

\subsubsection{Retorno como Restrição}

A forma mais direta de incorporar o retorno ao problema de otimização é tratá-lo como uma restrição, e não como parte da função objetivo. Nesse arranjo, a função a ser minimizada é exclusivamente a medida de risco, seja ela a variância da formulação de Markowitz ou o CVaR, enquanto o retorno entra como uma condição que a carteira precisa satisfazer. As formulações das equações~\eqref{eq:markowitz} e~\eqref{eq:cvar_opt} são exatamente desse tipo: em ambas, o termo $\mathbf{w}^\top \boldsymbol{\mu}$ aparece apenas nas restrições, fixando um retorno alvo $\mu^\ast$ ou impondo um retorno mínimo $\mu_{\min}$.

A vantagem dessa abordagem é que ela separa de forma limpa as duas grandezas. Em vez de decidir de antemão quanto retorno vale a pena trocar por quanto risco, o que exigiria escolher um peso relativo entre os dois, fixa-se um nível de retorno e pergunta-se qual é a carteira de menor risco capaz de atingi-lo. Cada valor escolhido para esse alvo define um problema de otimização distinto, cuja solução é um ponto no plano de risco-retorno. Variando o alvo ao longo de uma faixa de valores admissíveis e resolvendo o problema para cada um deles, obtém-se o conjunto de carteiras que formam a fronteira eficiente, conforme já descrito na Subseção~\ref{sec: med var}.

É importante notar que, nessa construção, o retorno esperado $\boldsymbol{\mu}$ permanece um dado de entrada, estimado externamente antes da otimização. A restrição $\mathbf{w}^\top \boldsymbol{\mu} = \mu^\ast$ é uma equação linear nos pesos, o que preserva a tratabilidade do problema: no caso da variância, mantém-se um problema quadrático com restrições lineares; no caso do CVaR linearizado por \citeonline{rockafellar2000optimization}, mantém-se um problema de programação linear. Toda a fragilidade discutida na Subseção~\ref{sec: retorno esperado}, ligada à dificuldade de estimar $\boldsymbol{\mu}$, continua presente, apenas deslocada para a etapa anterior à otimização. Tratar o retorno como restrição organiza o problema, mas não resolve a questão de onde vêm os números que alimentam essa restrição, assunto retomado na seção \ref{sec: CAPM}.
...
O índice de Sharpe carrega, no entanto, uma suposição implícita: a de que a distribuição dos retornos da carteira é aproximadamente normal, ou seja, que ela fica inteiramente descrita por apenas dois números, a média e o desvio padrão. Por conta dessa suposição, dois desvios de mesma magnitude, um para cima e outro para baixo, são igualmente prováveis e igualmente penalizados pela variância. Essa suposição é conveniente, mas nem sempre corresponde ao comportamento real dos retornos de ações, e nem à penalização que desejamos aplicar ao problema, dado que um desvio para cima em relação ao retorno médio é positivo e um para baixo é negativo, como discutido a seguir \cite{fabozzi2008portfolio}.

\subsubsection{Índice STARR}

O índice de Sharpe resume risco e retorno em um único número, mas o faz apoiado na variância, e por isso herda todas as limitações discutidas anteriormente: a suposição de normalidade e a penalização simétrica de desvios para cima e para baixo. Uma forma de manter a lógica da razão, retorno excedente por unidade de risco, sem carregar essas limitações, é substituir o denominador por uma medida de risco que capture o comportamento de cauda. É essa a proposta do índice STARR (Stable Tail Adjusted Return Ratio), que troca o desvio padrão pelo CVaR no denominador \cite{martin2003phi}:
\begin{equation}
\mathrm{STARR}_\alpha = \frac{\mathbf{w}^\top \boldsymbol{\mu} - r_f}{\mathrm{CVaR}_\alpha(\mathbf{w})},
\label{eq:starr}
\end{equation}
em que o numerador é o mesmo retorno excedente sobre o ativo sem risco usado no índice de Sharpe, e o denominador é o CVaR ao nível de confiança $\alpha$, definido na Subseção~\ref{sec: med var}. A leitura da razão é análoga à do Sharpe: quanto maior o STARR, mais retorno excedente a carteira entrega por unidade de risco de cauda assumido.

A diferença de comportamento entre os dois índices está inteiramente no denominador. Enquanto o Sharpe mede o risco pelo espalhamento total dos retornos em torno da média, tratando ganhos e perdas de forma indistinta, o STARR mede o risco apenas pela perda média nos piores cenários. Duas carteiras com o mesmo retorno esperado e a mesma variância, mas com caudas de perda diferentes, recebem o mesmo Sharpe e STARR distintos, sendo penalizada pelo STARR justamente aquela com maior exposição a perdas extremas. Por alinhar o denominador da razão à medida de risco central do projeto, o STARR é a forma natural de resumir risco e retorno em um único número dentro da abordagem baseada em CVaR, mantendo a coerência com a escolha de função objetivo feita nas seções anteriores.

\subsubsection{CAPM}
\label{sec: CAPM}
As formulações anteriores, tanto a restrição de retorno quanto as razões de Sharpe e STARR, partem do vetor de retornos esperados $\boldsymbol{\mu}$ como se ele fosse conhecido. Na prática, porém, esse é o ponto mais delicado de toda a construção, conforme adiantado na Subseção~\ref{sec: retorno esperado}. A média histórica dos retornos, embora seja a estimativa mais direta, é ruidosa e converge lentamente com o tamanho da amostra, o que produz carteiras instáveis e sensíveis a pequenas variações nos dados. Enquanto o risco de um ativo pode ser aproximado com relativa segurança a partir de uma amostra limitada, o seu retorno futuro é muito mais difícil de estimar, e é dessa dificuldade que decorre boa parte da fragilidade das carteiras obtidas por otimização crua.

Uma primeira resposta a esse problema veio de \citeonline{black1992global}, que em vez de estimar $\boldsymbol{\mu}$ livremente a partir do histórico de cada ativo, propõem ancorar essa estimativa em um ponto de referência vindo de um modelo de equilíbrio de mercado, o CAPM (Capital Asset Pricing Model), desenvolvido por \citeonline{sharpe1964capital} e \citeonline{lintner1965valuation}.

A ideia de equilíbrio funciona assim: se todo investidor decide de maneira racional e integralmente informada, ninguém opta por manter na própria carteira um ativo que parece ruim ou arriscado demais para o seu retorno, nem consegue comprar mais do que existe de um ativo que parece bom demais. Por isso, a quantidade de cada ativo que o conjunto de todos os investidores querem ter precisa bater com a quantidade que existe no mercado. Quando isso acontece, o mercado está em equilíbrio, e a distribuição das quantidades de tudo que existe para ser comprado acaba por configurar uma carteira eficiente.
...
\subsection{Modelos substitutos e o Processo de Otimização}
\label{sub:surrogate}

Cada carteira possível pode ser vista como um ponto de um mapa cuja altura representa o seu desempenho, por exemplo o CVaR. O objetivo da otimização é achar o pico mais alto ou o vale mais baixo, mas, como discutido até então, esse processo não é simples. A dificuldade é que não se enxerga o terreno inteiro: para conhecer a altura em um ponto é preciso medir, aplicando a função objetivo, e medir o mapa completo é caro. Como alternativa, seguir caminhos que descem ou sobem é uma abordagem que funciona apenas em terrenos bem comportados, e certas funções objetivo não garantem essa possibilidade. Com orçamento de medições limitado, a pergunta é como achar o ótimo gastando o mínimo de avaliações. A partir dessa mesma pergunta abrem-se dois caminhos, que se distinguem pelo momento em que a otimização acontece em relação à construção do modelo substituto.

\subsubsection{Metamodelo global e otimização posterior}

A primeira opção é avaliar uma amostra de pontos bem distribuídos no espaço, de modo a cobri-lo de forma significativa, e, partindo dessas medições, modelar uma superfície sobre a qual métodos de otimização comuns cheguem rapidamente a uma resposta. Essa superfície é o que se chama de modelo substituto ou metamodelo. No caso de uma carteira de investimentos, a abordagem equivale a gerar um número alto de combinações de pesos $\mathbf{w}$, avaliar o CVaR em cada uma delas, construir com esses pares uma aproximação da superfície de risco sobre todo o espaço admissível de $\mathbf{w}$ e, desse modelo substituto, extrair as informações desejadas. O resultado é uma correspondência $n$-dimensional, com $n$ o número de ativos, entre combinações de pesos e seus riscos.

Sobre essa correspondência é realizada a otimização, buscando o menor risco. Como o modelo substituto aproxima apenas o risco, o retorno permanece fora dele e é aplicado nesse passo posterior, na forma da restrição linear de retorno mínimo $\mathbf{w}^\top \boldsymbol{\mu} \geq \mu_{\min}$. Variando o valor de $\mu_{\min}$, cada nível de retorno exigido define um problema de risco mínimo distinto, e o conjunto das soluções descreve a fronteira eficiente. É essa separação, superfície de risco aproximada uma única vez e restrição de retorno imposta na otimização subsequente, que torna a fronteira barata de traçar: o metamodelo é construído uma vez e reaproveitado em vários problemas de otimização. Esse é, de forma simplificada, o método seguido por \citeonline{barrosa2015aplicacao}.

\subsubsection{Otimização bayesiana}

A segunda opção inverte a ordem entre modelar e otimizar. Em vez de gerar um modelo completo do espaço para só depois otimizar sobre ele, busca-se reduzir o número de medições necessárias otimizando sobre o metamodelo enquanto os próprios pontos de medição são escolhidos. Fazem-se algumas medições espalhadas e, a partir delas, constrói-se um palpite sobre a superfície inteira, inclusive onde não se mediu; a cada nova medição o palpite é refinado, e o critério que decide onde medir é o que dirige a busca em direção ao ótimo. Esse palpite barato de consultar é o modelo substituto.

O substituto mais usado é o processo gaussiano \cite{rasmussen2006gaussian}, que fornece, para cada ponto $\mathbf{w}$, não apenas uma média prevista $\hat{\mu}(\mathbf{w})$ (não confundir com o $\boldsymbol{\mu}$ do retorno esperado), mas também uma medida de incerteza $\hat{\sigma}(\mathbf{w})$ sobre essa previsão. Perto de onde se mediu, a incerteza é pequena; longe do ponto de medição, ela é grande. É essa informação de incerteza que orienta onde vale a pena medir em seguida, e é ela que distingue este caminho do anterior: aqui a construção do modelo e a busca pelo ótimo avançam juntas, guiadas pela incerteza, em vez de acontecerem em etapas separadas.

A escolha do próximo ponto cabe à função de aquisição, que combina a média prevista e a incerteza para pontuar cada candidato, equilibrando uma tensão entre a explotação e a exploração. A explotação consiste em medir onde o substituto prevê bom desempenho, apostando no que já parece promissor. A exploração consiste em medir onde o substituto está muito incerto, investigando o desconhecido, pois pode haver um ótimo escondido. Apenas explotar prende a busca em um ótimo local; apenas explorar desperdiça medições em regiões ruins. Esse compromisso é o tema central de \citeonline{moran2026balancing} e reaparece em \citeonline{millar2025bayesian} e \citeonline{you2025improving}.
...
\subsubsection{A restrição de retorno em cada abordagem}

As duas opções tratam a restrição de retorno de maneiras diferentes, e essa diferença está ligada à função objetivo escolhida. Quando o objetivo é uma medida de risco pura, como o CVaR, o VaR ou a variância, o retorno não aparece no que se minimiza; ele entra como a restrição $\mathbf{w}^\top \boldsymbol{\mu} \geq \mu_{\min}$, e é a variação de $\mu_{\min}$ que gera a fronteira eficiente. Esse é o cenário natural da primeira abordagem, na qual a restrição é imposta na otimização que roda sobre a superfície de risco já construída.

Quando o objetivo é um índice que combina risco e retorno em um único número, como o de Sharpe ou o STARR, o retorno já está embutido no próprio objetivo. Não há uma fronteira a traçar, e sim um único ótimo a encontrar, e a restrição linear de retorno deixa de figurar no problema, pois o compromisso entre risco e retorno passa a ser resolvido dentro da métrica. Resta apenas a restrição estrutural da carteira, $\sum_i w_i = 1$ e $w_i \geq 0$, já garantida na etapa de amostragem pela geometria do simplex, conforme discutido na Subseção~\ref{sub: doe}. É esse cenário, de um objetivo escalar único, custoso de avaliar e sem restrição de retorno explícita, que favorece a otimização bayesiana, cuja economia de medições é mais valiosa justamente quando cada avaliação da função objetivo é cara.

\subsubsection{Escolha do modelo substituto}

Convém distinguir dois eixos que as subseções anteriores mantiveram separados: o modelo substituto, isto é, com o que se aproxima a superfície, e o regime de otimização, isto é, se a otimização ocorre depois de construído o metamodelo ou simultaneamente à sua construção. São escolhas independentes. O processo gaussiano, por exemplo, atende aos dois regimes: no regime global ele cumpre o papel de interpolador, ao passo que no regime sequencial acrescenta a informação de incerteza que dirige a busca. De fato, a Krigagem empregada por \citeonline{barrosa2015aplicacao} é o mesmo objeto matemático que o processo gaussiano, aplicado no regime global e sem que sua incerteza seja utilizada.

A diferença entre os regimes recai, então, sobre a exigência que cada um impõe ao substituto. O regime sequencial depende de um modelo capaz de fornecer incerteza, pois é dela que a função de aquisição se alimenta; o regime global aceita qualquer interpolador, com ou sem incerteza, bastando que ele aproxime bem a superfície. É por isso que \citeonline{barrosa2015aplicacao} pôde empregar tanto a Krigagem quanto redes neurais como técnicas de interpolação: no regime global, a ausência de incerteza das redes neurais não é um obstáculo.

Sobre a escolha do substituto no regime sequencial, cabe notar que o processo gaussiano é o padrão na literatura justamente por fornecer a incerteza de forma natural e bem fundamentada, o que é essencial para a função de aquisição. Modelos da família de árvores, como florestas aleatórias e algoritmos de reforço de gradiente, não fornecem essa incerteza de forma imediata. \citeonline{you2025improving} recorrem a um estimador de estrutura em árvore, o TPE, que constrói a incerteza de outra maneira e escala melhor que o processo gaussiano sob orçamento apertado. Testar tais alternativas é uma linha legítima, condicionada à capacidade de o substituto entregar incerteza utilizável pela função de aquisição, ou condicionada à mudança da função de aquisição. Adotar o processo gaussiano como substituto comum a ambos os regimes, por fim, mantém o interpolador constante entre o baseline global e a otimização bayesiana, isolando o processo de otimização como a variável efetivamente sob estudo.


\subsubsection{Processos Gaussianos aplicados a portfólios}
